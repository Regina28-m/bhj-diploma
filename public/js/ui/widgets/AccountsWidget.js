...
if(!element) throw new Error('Элемент не существует');
 ...
 if(User.current()) {
   ...
   * */
  clear() {
this.element.querySelectorAll('.account').forEach(acc => {
...
const currentAccount = element.closest('.account');
    ...
    App.showPage('transactions', {account_id: currentAccount.dataset.id});
  }
...
return `<li class="account" data-id="${item.id}">
   ...
   * */
  renderItem(data){
data.forEach(acc => this.element.insertAdjacentHTML('beforeend',this.getAccountHTML(acc)));
  }
}
